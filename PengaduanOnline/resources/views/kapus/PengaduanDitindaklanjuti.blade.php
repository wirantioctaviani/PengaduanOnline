            ...
            <div class="col-md-12 border bg-light" style="height: 100%">
                ...
                <br>
                  <table id="example" class="table table-striped table-bordered dt-responsive">
                    ...
                    <tbody>
                        @foreach($pengaduan_new as $pengaduan_ditindaklanjuti)
                        <tr class="text-center">
                            ...
                            <td>{{date("d M Y H:i", strtotime($pengaduan_ditindaklanjuti->kapus_updated_at))}}</td>
                            ...
                            @if($pengaduan_ditindaklanjuti->status_pic == 1 || $pengaduan_ditindaklanjuti->status_pic == 2)
                                <td>
                                <a href="{{ action('App\Http\Controllers\KapusController@ProsesPengaduan', [encrypt($pengaduan_ditindaklanjuti->id)]) }}" class="btn btn-warning">Update</a>
                                </td>
                            @elseif($pengaduan_ditindaklanjuti->status_pic == 3 || $pengaduan_ditindaklanjuti->status_pic == 4)
                                <td>Anda telah menyelesaikan proses tindak lanjut<br>
                                  <br>
                                <a href="{{ action('App\Http\Controllers\KapusController@ProsesPengaduan', [encrypt($pengaduan_ditindaklanjuti->id)]) }}" class="btn btn-info">Detail</a>
                                </td>
                            @endif
                        </tr>
                        @endforeach
