...
class CreateTrhKoordinatorTable extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::create('trh_koordinator', function (Blueprint $table) {
            $table->bigIncrements('idt_koordinator');
            ...
            //         ->onUpdate('cascade');
            $table->integer('koor_id')->unsigned();
            $table->foreign('koor_id')->references('id')->on('users')
            ->onUpdate('cascade');
    ...
    {
        Schema::dropIfExists('trh_koordinator');
    }
}
